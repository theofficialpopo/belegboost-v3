...
const AdvisorFinalizeStep: React.FC<AdvisorFinalizeStepProps> = ({ data, updateData }) => {
  ...
  const filteredAdvisors = useMemo(
    () => ADVISORS.filter(a => a.name.toLowerCase().includes(search.toLowerCase())),
    [search]
  );
  ...
  const endBalanceNum = useMemo(() => parseCurrencyToNumber(data.endBalance), [data.endBalance]);
  const calculatedStartBalance = useMemo(
    () => endBalanceNum - mockIncoming + mockOutgoing,
    [endBalanceNum]
  );

  const handleAdvisorSelect = useCallback((advisorName: string) => {
    updateData({ selectedAdvisor: advisorName });
  }, [updateData]);

  const dateRangeDisplay = useMemo(() => {
    if (data.startDate && data.endDate) {
      return `${formatDateDE(data.startDate)} - ${formatDateDE(data.endDate)}`;
    }
    return '-';
