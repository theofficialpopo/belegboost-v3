...
const PeriodBalanceStep: React.FC<PeriodBalanceStepProps> = ({ data, updateData, onNext }) => {
  ...
  const setPreset = (type: 'lastMonth' | 'prevLastMonth' | 'lastQuarter' | 'year') => {
            ...
            <div className="space-y-6">
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2 text-xs">
                        <button 
                            onClick={() => { setMode('month'); updateData({startDate: '', endDate: ''}); }}
                        ...
                        <button 
                            onClick={() => { setMode('range'); updateData({startDate: '', endDate: ''}); }}
                ...
                <div className="flex flex-wrap gap-2">
                    {[{l:'Letzter Monat', v:'lastMonth'}, {l:'Vorletzter Monat', v:'prevLastMonth'}, {l:'Letztes Q.', v:'lastQuarter'}, {l:'Letztes Jahr', v:'year'}].map((btn) => (
                        <button 
                            key={btn.v}
                            onClick={() => setPreset(btn.v as any)} 
            ...
            <div className="flex flex-col">
                 ...
                 <div className="flex-grow flex flex-col justify-center p-8 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700 hover:border-primary-400 transition-colors">
                    <div className="text-center mb-4 text-sm text-slate-500">
                        Bitte geben Sie den Endsaldo des Kontoauszugs zum <strong className="text-slate-900 dark:text-white">{formatDateDE(data.endDate) || 'Enddatum'}</strong> ein.
                    </div>
